...
import (
	"bytes"
	"encoding/json"
	"gin-splitwise/internal/account/database/mocks"
...
func (s *HandlerSuite) TestCurrentUser() {
	...
	// when
	res := httptest.NewRecorder()
	req, _ := http.NewRequest("GET", "/v1/api/user/profile", nil)
...
func (s *HandlerSuite) TestUpdate() {
	...
	res := httptest.NewRecorder()
	req, _ := http.NewRequest("PUT", "/v1/api/users/profile", bytes.NewBuffer(b))
	req.Header.Add("Authorization", "Bearer "+token)
	...
	s.db.AssertCalled(s.T(), "Update", mock.Anything, acc.Email, mock.MatchedBy(func(a *model.Account) bool {
		return a.Email == acc.Email && a.Username == "updated-user1" && a.Image == "updated-image"
	}))
	s.Equal(http.StatusOK, res.Code)
	expected := `
	{
