...
func (s *DBSuite) TestSave() {
	...
	// when
	now := time.Now()
	err := s.db.Save(context.TODO(), &acc)

	// then
	s.NoError(err)
	find, err := s.db.FindByEmail(context.TODO(), acc.Email)
	s.NoError(err)
...
func (s *DBSuite) TestSave_ErrorIfExistEmail() {
	...
	err := s.db.Save(context.TODO(), &model.Account{
		Username: "user1",
		Email:    email,
		Password: "pass",
	})
...
func (s *DBSuite) TestUpdate() {
	...
	s.NoError(s.db.Save(context.TODO(), &acc))
	...
	// when
	err := s.db.Update(context.TODO(), acc.Email, updated)
...
func (s *DBSuite) TestUpdate_FailIfNotExist() {
	// when
	err := s.db.Update(context.TODO(), "unknown@emai.com", &model.Account{
		Username: "updated",
	})
...
func (s *DBSuite) TestFindByEmail_ErrorIfNotExist() {
	// when
	find, err := s.db.FindByEmail(context.TODO(), "unknown@email.com")
