...
// NewTestDatabase starts a PostgreSQL docker container and returns *gorm.DB
func NewTestDatabase(tb testing.TB, migration bool) *gorm.DB {
	...
	if err != nil {
		tb.Fatalf("Failed to set expiration for the resource: %v", err)
	}

	// exponential backoff-retry, because the application in the container might not be ready to accept connections yet
	dcn := fmt.Sprintf("postgres://postgres:secret@localhost:%s/testdb?sslmode=disable", resource.GetPort("5432/tcp"))
